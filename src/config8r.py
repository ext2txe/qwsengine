# config8r_v3.py
# Adds a vertical splitter inside the Listing Analyzer dock: top=candidates, bottom=matches.
# File path bar stays fixed above the splitter. No central HTML viewer.
...
from PySide6.QtWidgets import (
    ...
    QToolBar, QStatusBar, QLabel, QListWidget, QSplitter
)
...
class Config8rWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Config8r – Listing Analyzer v3")
        self.resize(1150,750)
        self.project={"listing":{},"detail":{"fields":[]},"custom_processors":[]}
        self.current_listing_path=None; self.current_doc=None
    ...
    def _build_listing_analyzer_dock(self):
        ...
        body=QWidget(self.dckListingAnalyzer); v=QVBoxLayout(body)
        # --- Top path bar (fixed) ---
        ...
        # Top: candidates box with table + buttons
        topWidget=QWidget(); tv=QVBoxLayout(topWidget)
        grp=QGroupBox("Item candidates",topWidget); gv=QVBoxLayout(grp)
        ...
        grp.setLayout(gv); tv.addWidget(grp); topWidget.setLayout(tv)

        # Bottom: matches label + list
        bottomWidget=QWidget(); bv=QVBoxLayout(bottomWidget)
        self.lblMatches=QLabel("Matches (select a candidate)",bottomWidget)
        self.lstMatches=QListWidget(bottomWidget)
        bv.addWidget(self.lblMatches); bv.addWidget(self.lstMatches,1)
        bottomWidget.setLayout(bv)

        splitter.addWidget(topWidget); splitter.addWidget(bottomWidget)
        splitter.setSizes([420, 300])  # default ~60/40
        v.addWidget(splitter,1)

        body.setLayout(v); self.dckListingAnalyzer.setWidget(body)
    ...
    def _show_matches_for_css(self,css):
        ...
        for node in nodes:
            ...
            try: txt=" ".join(" ".join(node.itertext()).split())
            except Exception: txt=""
            try: frag=lxml_html.tostring(node,encoding="unicode")[:200].replace("\\n"," ")
            except Exception: frag=""
