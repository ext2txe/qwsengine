...
from poc import detect_repeating_items, infer_selector_from_clicks
from scopes import resolve_scope_nodes, evaluate_selector, extract_value
from processors import run_pipeline, build_context
...
class Config8rWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.dev_mode = False
        self.custom_registry = {}  # filled only when dev_mode ON
        self.anchor_cache = {}
        self._build_ui()
        self._wire_signals()
    
        self.setWindowTitle("Config8rWindow  – Detail Page Capture & Extractor Generator")
    ...
    def on_click_analyze_listing(self):
        dom_root = self.webview_bridge.dom_root()  # whatever returns the root/handle
        candidates = detect_repeating_items(dom_root)
        self.listingCandidatesView.setItems(candidates)  # your sidebar list
