...
from PySide6 import QtWebEngineCore as qtwc
...
class MainWindow(qtw.QMainWindow):
    ...
    def __init__(self):
        ...
        pixmapi = getattr(qtw.QStyle.StandardPixmap, "SP_ArrowBack")
        self.back.setIcon(style.standardIcon(pixmapi))

        self.forward = navigation.addAction('Forward')
        #self.forward.setIcon(style.standardIcon(style.SP_ArrowForward))
        pixmapi = getattr(qtw.QStyle.StandardPixmap, "SP_ArrowForward")
        self.forward.setIcon(style.standardIcon(pixmapi))

        self.reload = navigation.addAction('Reload')
        #self.reload.setIcon(style.standardIcon(style.SP_BrowserReload))
        pixmapi = getattr(qtw.QStyle.StandardPixmap, "SP_BrowserReload")
        self.reload.setIcon(style.standardIcon(pixmapi))

        self.stop = navigation.addAction('Stop')
        #self.stop.setIcon(style.standardIcon(style.SP_BrowserStop))
        pixmapi = getattr(qtw.QStyle.StandardPixmap, "SP_BrowserStop")     
        self.stop.setIcon(style.standardIcon(pixmapi))
        ...
        pixmapi = getattr(qtw.QStyle.StandardPixmap, "SP_DialogOkButton")
        self.go.setIcon(style.standardIcon(pixmapi))
        ...
        # Profile sharing
        self.profile = qtwc.QWebEngineProfile()
        ...
        # Altering Settings
        settings = qtwc.QWebEngineSettings  # was settings = qtwc.QWebEngineSettings.defaultSettings()
        #settings.setFontFamily(qtwc.QWebEngineSettings.SansSerifFont, 'Impact')
        #settings.setAttribute(qtwc.QWebEngineSettings.PluginsEnabled, True)
    ...
    def add_tab(self, *args):
        ...
        # Profile
        page = qtwc.QWebEnginePage(self.profile)
